...
lang: en
permalink: /en/insights/inside-zazoon-modular-grc/
alternate_url: /insights/inside-zazoon-modular-grc/
standfirst: "A durable software platform earns its place by matching the shape of the demand it serves. We backed MyGRC because its modular architecture does exactly that."
summary: "Why Northstone made Zazoon its inaugural investment: a shared-model modular software platform that compounds, absorbing needs the way they actually accrue — one at a time."
---

Northstone made Zazoon its inaugural investment for a reason that has less to do with any single feature than with shape. MyGRC is a modular software platform — its modules an Internal Control System, Risk Management, and Data Protection can each be bought and run on their own, yet sit on one shared data model. We did not back that design because modular software is in fashion. We backed it because it is the kind of durable, recurring-revenue software business our thesis is built around, and because its architecture matches how the demand it serves actually reaches an organisation: not as a single procurement event, but one need at a time, years apart, each on its own clock and rarely coordinated with the last. That those needs happen to be regulatory obligations sharpens the case; it is not the whole of it.
